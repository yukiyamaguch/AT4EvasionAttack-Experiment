...
for j in range(len(testX)):
    ...
    ae=np.clip(perturbation, -1,1)
    aeimg = np.clip(ae + image, 0, 1)
    # white
    #pred = model.predict(aeimg)
    # proxy
    pred = victim.predict(aeimg)
    print('60kmL Preds: {}'.format(pred[0][3]))
    print('Stop  Preds: {}'.format(pred[0][14]))
    print(f'argmax: {np.argmax(pred[0])}')
    print(f'Norm: {l2_norm_of_perturbation(ae)}')
    ae255 = 255*cv2.resize(ae[0], (256,256))
    output = np.clip(imgs[j] + ae255, 0, 255)
    write_img(os.path.join(path, f'AE-{count:02}.png'), output)
    count += 1
    if np.argmax(pred[0]) == 14:
        acc+=1
    norm += l2_norm_of_perturbation(ae)
    print()
