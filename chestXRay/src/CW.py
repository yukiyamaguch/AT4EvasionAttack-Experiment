...
parser = argparse.ArgumentParser(description='Calini and Wagner Attack test program')
parser.add_argument('--AK', choices=['white', 'proxy'], help='Attacker\'s Knowledge.')
parser.add_argument('--con', type=float, default=0.1, help='hyperparameter')
...
# generate AE
attack = CarliniL2Method(classifier=classifier, confidence=confidence, targeted=True)
testAE = attack.generate(x=testX, y=to_categorical(len(testY)*[0], num_classes=2))
#attack = CarliniLInfMethod(classifier=classifier, confidence=confidence)
...
print(f'Hyperparameter con: {confidence}')

temp = np.clip(255*testAE[0], 0, 255)
write_img(os.path.join(args.output_path, f'con{confidence}.png'), temp)
