...
parser = argparse.ArgumentParser(description='AdvPatch test program')
parser.add_argument('--AK', choices=['white', 'proxy'], help='Attacker\'s Knowledge.')
parser.add_argument('--scale', type=float, default=0.2, help='hyperparameter')
...
# generate AE
attack = AdversarialPatch(classifier=classifier, learning_rate=0.1, scale_min=0.1, scale_max=0.4, rotation_max=50, targeted=True)
#mask = np.array([[False if (i > 15 and i < 25) else True for i in range(28)] if (j > 15 and j < 25) else [True for i in range(28)] for j in range(28)] )
#mask = np.reshape(mask, (1,28,28))
#testAE = attack.generate(x=testX, y=np.array(len(testY)*[9]), mask=mask)
testAE = attack.generate(x=testX, y=np.array(len(testY)*[9]))
testAE = attack.apply_patch(x=testX, scale=scale)
...
print(f'Hyperparameter scale: {scale}')

temp = np.clip(255*testAE[0], 0, 255)
write_img(os.path.join(args.output_path, f'scale{scale}.png'), temp)
